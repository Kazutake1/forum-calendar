...
def _tsv_words(tsv_text):
    """Parse Tesseract TSV into word dictionaries."""
    rows=[]
    ...
    for raw in lines[1:]:
        ...
        if len(parts)<len(header):
            continue
        try:
            if int(parts[idx["level"]]) != 5:
                continue
            word=parts[idx["text"]].strip()
            if not word:
                continue
...
def _group_tsv_lines(words):
    ...
    return sorted(out,key=lambda z:(z["page"],z["y"]))

def _coord_day(words):
    # First prefer a compact "17日" token.
    for i,w in enumerate(words[:5]):
        m=re.fullmatch(r"(\d{1,2})\s*日",w["text"])
        if m and 1<=int(m.group(1))<=31:
            return int(m.group(1)),i
    # Then allow "17" + "日" split into neighboring tokens.
    for i,w in enumerate(words[:5]):
        if re.fullmatch(r"\d{1,2}",w["text"]):
            d=int(w["text"])
            if not 1<=d<=31:
                continue
            if i+1<len(words) and words[i+1]["text"]=="日":
                return d,i
    return None,None

def _coord_weekday_index(words,start_i=0):
    pat=re.compile(r"^(?:月|火|水|木|金|土|日)(?:[・･]?祝)?$")
    for i in range(start_i,min(len(words),start_i+6)):
        t=words[i]["text"].replace(" ","")
        if pat.fullmatch(t):
            return i
    return None

def _coord_hall_index(words,weekday_i):
    """Find the row-local 大/中/小 marker near the weekday cell."""
    mapping={"大":"大ホール","中":"中ホール","小":"小ホール"}

    # Strongest: exact one-character token immediately after weekday.
    for i in range(weekday_i+1,min(len(words),weekday_i+5)):
        t=words[i]["text"].strip("|｜[]() ")
        if t in mapping:
            return mapping[t],i

    # OCR may attach borders or punctuation to the marker.
    for i in range(weekday_i+1,min(len(words),weekday_i+5)):
        t=words[i]["text"]
        m=re.search(r"(^|[|｜\[\(\s])([大小中])($|[|｜\]\)\s])",t)
        if m:
            return mapping[m.group(2)],i

    return "",None

def _coord_title(words,start_i):
    """Build a title from words after the hall marker until time/admission columns."""
    if start_i is None or start_i>=len(words):
        return ""
    bits=[]
    for w in words[start_i:]:
        t=w["text"].strip()
        if not t:
            continue
        # Stop when the right-side time/admission portion begins.
        if re.search(r"\b\d{1,2}:\d{2}\b",t):
            break
        if t in {"無料","関係者","要整理券","会員制","見学可"}:
            break
        bits.append(t)
    title=clean(" ".join(bits),240)
    title=title.strip("|｜ +-・･")
    compact=re.sub(r"\s","",title)
    if len(compact)<4:
        return ""
    if compact in {"催事名","催事予定表","入場方法","開演時間","主催者問合先"}:
        return ""
    if re.fullmatch(r"[|｜年月火水木金土日祝大小中0-9:+〜~\-]+",compact):
        return ""
    return title

def _coord_time_and_price(line_text):
    tm=re.search(r"(\d{1,2}:\d{2})(?:\s*[〜～~-]\s*(\d{1,2}:\d{2}))?",line_text)
    time=tm.group(1)+(f"〜{tm.group(2)}" if tm and tm.group(2) else "〜") if tm else ""
    price="無料" if "無料" in line_text else "関係者" if "関係者" in line_text else "要整理券" if "要整理券" in line_text else ""
    return time,price

def parse_schedule_coordinates(pdf_bytes,year,months):
    """Coordinate parser with multi-line row merging.

    The schedule PDF often splits one visual event across several OCR lines:
      line A: date / weekday / hall
      line B: event title
      line C: time / admission / organizer
    This parser groups nearby lines by vertical position and keeps the hall marker
    from the event's own row, never from a previous event.
    """
    events=[]
    debug=[]
    with tempfile.TemporaryDirectory() as td:
        pdf=Path(td)/"schedule.pdf"
        pdf.write_bytes(pdf_bytes)
        prefix=Path(td)/"coord"
        subprocess.run(
            ["pdftoppm","-png","-r","300",str(pdf),str(prefix)],
            check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=120
        )
        ...
        for img in sorted(Path(td).glob("coord-*.png")):
            best_lines=[]
            best_score=-1
            ...
            for psm in ("6","4"):
                try:
                    p=subprocess.run(
                        ["tesseract",str(img),"stdout","-l","jpn+eng","--psm",psm,"tsv"],
                        capture_output=True,text=True,check=True,timeout=120
                    )
                    lines=_group_tsv_lines(_tsv_words(p.stdout))

                    # Score by how many date/weekday/hall rows are visible.
                    score=0
                    for ln in lines:
                        d,di=_coord_day(ln["words"])
                        if d is None:
                            continue
                        wi=_coord_weekday_index(ln["words"],di+1)
                        if wi is None:
                            continue
                        hall,hi=_coord_hall_index(ln["words"],wi)
                        if hall:
                            score += 10
                    if score > best_score:
                        best_score=score
            ...
            # Detect explicit month headers first.
            for ln in best_lines:
                mm=re.search(r"(?<!\d)(\d{1,2})\s*月",clean(ln["text"],800))
            ...
            while i < len(best_lines):
                ln=best_lines[i]
                words=ln["words"]
                line_text=clean(ln["text"],1200)

                day,day_i=_coord_day(words)
                if day is None:
                    i += 1
                    continue

                weekday_i=_coord_weekday_index(words,day_i+1)
                if weekday_i is None:
                    i += 1
                    continue

                hall,hall_i=_coord_hall_index(words,weekday_i)
                if not hall:
                    if len(debug)<120:
                        debug.append(f"COORD MISS p{ln['page']} y{ln['y']}: {line_text}")
                    i += 1
                    continue

                # Start with any title fragment remaining on the date/hall row.
                fragments=[]
                title0=_coord_title(words,hall_i+1)
                if title0:
                    fragments.append(title0)

                combined_text=line_text

                # Merge following OCR lines that belong to the same event block.
                # Stop at the next row that itself begins with a date.
                j=i+1
                base_y=ln["y"]
                while j < len(best_lines):
                    nxt=best_lines[j]
                    if nxt["page"] != ln["page"]:
                        break

                    nd,ndi=_coord_day(nxt["words"])
                    if nd is not None:
                        break

                    # Keep only nearby lines. At 300 dpi this comfortably covers
                    # wrapped title/time/organizer lines while avoiding the next block.
                    if nxt["y"] - base_y > 170:
                        break

                    ntext=clean(nxt["text"],1200)
                    combined_text += " " + ntext

                    # Title candidates from continuation lines:
                    # reject lines that are obviously time/price/phone-only metadata.
                    compact=re.sub(r"\s+"," ",ntext).strip()
                    if compact:
                        metadata_only = (
                            re.fullmatch(r"[\d:\-〜～\s|｜]+",compact) or
                            re.search(r"^(無料|関係者|要整理券|会員制|見学可)\b",compact) or
                            re.fullmatch(r"(?:0\d{1,4}[-ー]\d{1,4}[-ー]\d{3,4}|\d{2,4}[-ー]\d{2,4}[-ー]\d{3,4})",compact)
                        )
                        if not metadata_only:
                            # Remove obvious right-side metadata tokens.
                            cand=re.sub(r"\b\d{1,2}:\d{2}\b.*$","",compact)
                            cand=re.sub(r"\b(無料|関係者|要整理券|会員制|見学可)\b.*$","",cand)
                            cand=clean(cand,240).strip("|｜ +-・･")
                            if len(re.sub(r"\s","",cand)) >= 4:
                                fragments.append(cand)
                    j += 1

                # Merge title fragments, preserving order but removing exact duplicates.
                merged_parts=[]
                for f in fragments:
                    f=clean(f,240)
                    if f and f not in merged_parts:
                        merged_parts.append(f)
                title=clean(" ".join(merged_parts),240).strip("|｜ +-・･")

                # Reject fragments without a plausible event name.
                compact_title=re.sub(r"\s","",title)
                if (
                    len(compact_title)<4 or
                    compact_title in {"催事名","催事予定表","入場方法","開演時間","主催者問合先"} or
                    re.fullmatch(r"[|｜年月火水木金土日祝大小中0-9:+〜~\-]+",compact_title)
                ):
                    if len(debug)<120:
                        debug.append(f"COORD NO TITLE p{ln['page']} y{ln['y']}: {combined_text[:500]}")
                    i=j
                    continue

                try:
                    date=datetime(year,current_month,day).strftime("%Y-%m-%d")
                except ValueError:
                    i=j
                    continue

                time,price=_coord_time_and_price(combined_text)
                e={
                    "date":date,
                    "hall":hall,
                    "time":time,
                ...
                if len(debug)<120:
                    debug.append(
                        f"COORD OK p{ln['page']} y{ln['y']}: "
                        f"{date} {hall} | {title} | {time} | {price}"
                    )

                i=j

    print("=== COORDINATE OCR DEBUG START ===")
    for x in debug:
        print(x)
    print("=== COORDINATE OCR DEBUG END ===")
...
def main():
    ...
    try:
        ...
        coord_parsed=parse_schedule_coordinates(pdf,year,months)
        text_parsed=parse_schedule_ocr(extracted_text,year,months)

        # Coordinate results are preferred. If too few were recognized,
        # retain the text parser only for comparison; the normal trust gate below
        # still decides whether replacement is safe.
        if len(coord_parsed) >= len(text_parsed):
            parsed=coord_parsed
            notes.append(f"催事予定表解析方式: coordinate {len(coord_parsed)}件 / text {len(text_parsed)}件")
        else:
            parsed=text_parsed
            notes.append(f"催事予定表解析方式: text {len(text_parsed)}件 / coordinate {len(coord_parsed)}件")
        ...
        quality_ok = len(parsed)>=threshold and len(hall_kinds)>=3 and junk==0
        if quality_ok:
            schedule=parsed; schedule_trusted=True; source_success=True; forum_success=True
            notes.append(f"催事予定表OCR[座標版] 信頼済み {len(parsed)}件 / しきい値{threshold} / 会場{len(hall_kinds)}種")
        else:
            notes.append(f"催事予定表OCR[座標版] 信頼度不足 {len(parsed)}件 / しきい値{threshold} / 会場{len(hall_kinds)}種 / junk{junk} のため既存月データ維持")
    except Exception as e:
