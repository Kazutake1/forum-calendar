...
def main():
    ...
    try:
        ...
        if quality_ok:
            ...
            notes.append(f"催事予定表OCR 信頼済み {len(parsed)}件 / しきい値{threshold} / 会場{len(hall_kinds)}種")
        else:
            notes.append(f"催事予定表OCR 信頼度不足 {len(parsed)}件 / しきい値{threshold} / 会場{len(hall_kinds)}種 / junk{junk} のため既存月データ維持")
    except Exception as e:
