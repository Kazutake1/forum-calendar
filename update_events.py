...
def normalize_schedule_ocr(s):
    """Normalize OCR/table text while preserving row/column clues."""
    s=str(s or "").replace("\u3000"," ")
...
def schedule_hall_from_row(line):
    """Read the hall marker belonging to the event row itself.

    The schedule PDF frequently OCRs rows like:
      | 土 大 APF ... |
      | 水 中 令和8年度... |
      | 火 小 ... |
    A row-local marker is safer than inheriting the previous hall heading.
    """
    # Full hall names are strongest.
    m=re.search(r"(大ホール|中ホール|小ホール)", line)
    if m:
        return m.group(1)

    # The one-character marker normally follows weekday / holiday notation.
    # Accept separators produced by pdftotext/Tesseract.
    m=re.search(
        r"(?:^|[|｜\s])(?:月|火|水|木|金|土|日)(?:[・･]\s*祝)?\s*[|｜]?\s*([大小中])(?=[|｜\s])",
        line
    )
    if m:
        return {"大":"大ホール","中":"中ホール","小":"小ホール"}[m.group(1)]

    # OCR sometimes loses the weekday but leaves a date/day separator before the hall marker.
    m=re.search(r"(?:^|[|｜])\s*(?:\d{1,2}\s*日?\s*)?([大小中])(?=[|｜\s])", line)
    if m:
        return {"大":"大ホール","中":"中ホール","小":"小ホール"}[m.group(1)]
    return ""

def schedule_row_title(line, day, hall):
    """Remove table metadata and return only a plausible event title."""
    t=line

    # Remove the date cell once.
    t=re.sub(rf"(^|[|｜\s]){day}\s*日?(?=[|｜\s])"," ",t,count=1)

    # Remove weekday / holiday cell and its row-local hall marker.
    t=re.sub(
        r"(?:^|[|｜\s])(?:月|火|水|木|金|土|日)(?:[・･]\s*祝)?\s*[|｜]?\s*[大小中](?=[|｜\s])",
        " ",t,count=1
    )

    # Remove full hall names and standalone hall marker left by OCR.
    for h in HALLS:
        t=t.replace(h," ")
    t=re.sub(r"(^|[|｜\s])[大小中](?=[|｜\s])"," ",t,count=1)

    # Remove time/admission cells from the right side.
    t=re.sub(r"\b\d{1,2}:\d{2}\b.*$","",t)
    t=re.sub(r"\b(無料|関係者|要整理券|会員制)\b.*$","",t)

    # Remove table borders/noise.
    t=t.replace("|"," ").replace("｜"," ")
    t=re.sub(r"\s+"," ",t).strip(" +-・･")

    # Reject fragments that are not event names.
    compact=re.sub(r"\s","",t)
    if not compact or len(compact)<4:
        return ""
    if compact in {"催事名","催事予定表","入場方法","開演時間","終演時間"}:
        return ""
...
def parse_schedule_ocr(text, year, months):
    """Parse the hall schedule using row-local hall markers.

    Important: do NOT inherit the previous hall. The uploaded events.json showed
    that inheritance caused many rows to be incorrectly classified as 小ホール.
    """
    events=[]
    current_month=months[0]

    # Some PDF extractors split a logical table row across physical lines.
    # Keep a small pending event row so a following time-only line can enrich it,
    # but never create a new event from a time-only fragment.
    pending=None
    ...
    for raw in normalize_schedule_ocr(text).splitlines():
        ...
        hall=schedule_hall_from_row(line)

        # A line with a time but no date/title may be the continuation of the
        # immediately preceding event row. Only enrich; never create an event.
        if not dm:
            if pending:
                tm=re.search(r"(\d{1,2}:\d{2})(?:\s*[〜~-]\s*(\d{1,2}:\d{2}))?",line)
                if tm and not pending.get("time"):
                    pending["time"]=tm.group(1)+(f"〜{tm.group(2)}" if tm.group(2) else "〜")
                if "無料" in line and not pending.get("price"):
                    pending["price"]="無料"
                elif "関係者" in line and not pending.get("price"):
                    pending["price"]="関係者"
                elif "要整理券" in line and not pending.get("price"):
                    pending["price"]="要整理券"
            continue

        day=int(dm.group(1))
        if not (1 <= day <= 31) or not hall:
            pending=None
            continue
        ...
        title=schedule_row_title(line, day, hall)
        if not title:
            # This is typically a time/admission fragment such as "| 水 大 |".
            # Do not emit it as an event.
            pending=None
            continue
