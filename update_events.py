...
def expand_dates_from_text(text):
    ...
    # Reiwa dates
    for ry,m,d in re.findall(r"令和\s*(\d+)年\s*(\d{1,2})月\s*(\d{1,2})日", text):
        try: found.append(datetime(2018+int(ry),int(m),int(d)).date())
        except ValueError: pass
    # Expand a simple first-to-second range when it is reasonable
...
def main():
    ...
    guide=[]; schedule=[]; park=[]; schedule_trusted=False; target_yms=set(); source_success=False
    ...
    try:
        ...
        if checked>0: source_success=True
