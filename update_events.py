...
def parse_schedule_coordinates(pdf_bytes,year,months):
    """Use Tesseract word coordinates to parse each schedule row.

    This avoids depending on pdftotext's broken character order.
    The hall is read only from the same visual row as the event.
    """
    events=[]
    debug=[]
    with tempfile.TemporaryDirectory() as td:
        ...
        for img in sorted(Path(td).glob("coord-*.png")):
            best_lines=[]
            best_score=-1
            # PSM 6 usually keeps rows; PSM 4 sometimes understands tables better.
            for psm in ("6","4"):
                try:
                    p=subprocess.run(
            ...
            for ln in best_lines:
                ...
                mm=re.search(r"(?<!\d)(\d{1,2})\s*月",line_text)
                if mm and int(mm.group(1)) in months:
                    current_month=int(mm.group(1))

                day,day_i=_coord_day(ln["words"])
                if day is None:
                    continue
                weekday_i=_coord_weekday_index(ln["words"],day_i+1)
                if weekday_i is None:
                    continue
                hall,hall_i=_coord_hall_index(ln["words"],weekday_i)
                if not hall:
                    if len(debug)<80:
                        debug.append(f"COORD MISS p{ln['page']} y{ln['y']}: {line_text}")
                    continue

                title=_coord_title(ln["words"],hall_i+1)
                if not title:
                    continue
                try:
                    ...
                    continue
                time,price=_coord_time_and_price(line_text)
                e={
                    "date":date,
                    "hall":hall,
                    "time":time,
                ...
                events.append(e)
                if len(debug)<80:
                    debug.append(f"COORD OK p{ln['page']} y{ln['y']}: {date} {hall} | {title}")
