...
def page_is_valid(url, kind):
    try:
        ...
        if kind=="schedule":
            if "ホール催事予定表" in text:
                return True
            # Known official page fallback: accept if it contains at least one PDF link.
            return any(urlparse(urljoin(url,a.get("href",""))).path.lower().endswith(".pdf")
                       for a in soup.find_all("a", href=True))
        if kind=="events":
            if ("コンサート" in text and "イベント" in text) or "開催日" in text:
                return True
...
def find_schedule_pdf(schedule_page):
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "催事予定表" in label or "イベント" in label or re.search(r"20\d{2}[-_]?\d{2}", u.path):
            links.append(item)
    if links:
        return links[0]
    if fallback:
